...
CDeleteTab4::CDeleteTab4(CWnd* pParent /*=nullptr*/)
	: CDialogEx(IDD_DELETE_TAB4, pParent)
{
...
void CDeleteTab4::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_DELETE4_LIST, delete4_list);
...
BEGIN_MESSAGE_MAP(CDeleteTab4, CDialogEx)
	ON_BN_CLICKED(IDC_NOT_AGREE_BTN, &CDeleteTab4::OnBnClickedNotAgreeBtn)
	ON_BN_CLICKED(IDC_AGREE_BTN, &CDeleteTab4::OnBnClickedAgreeBtn)
	ON_BN_CLICKED(IDC_GOOD_BTN, &CDeleteTab4::OnBnClickedGoodBtn)
	ON_BN_CLICKED(IDC_DELETE4_QUERY_BTN, &CDeleteTab4::OnBnClickedDelete4QueryBtn)
	ON_NOTIFY(NM_DBLCLK, IDC_DELETE4_LIST, &CDeleteTab4::OnNMDblclkDelete4List)
END_MESSAGE_MAP()
...
void CDeleteTab4::OnBnClickedDelete4QueryBtn()
{
	delete4_list.SetExtendedStyle(LVS_EX_FULLROWSELECT);
	delete4_list.DeleteAllItems();
	...
	if (SQLAllocHandle(SQL_HANDLE_STMT, hDbc, &hStmt) == SQL_SUCCESS)
	{
		...
		for (int i = 0; i < colCount; ++i) {
			...
			delete4_list.InsertColumn(i, (char*)columnName[i], LVCFMT_CENTER, 150);
		}

		int row_item;
		while (SQLFetch(hStmt) != SQL_NO_DATA)
		{
			row_item = delete4_list.InsertItem(0, (char*)data[0]);
			for (int i = 1; i < colCount; ++i) {
				delete4_list.SetItemText(row_item, i, (char*)data[i]);
			}
		}
...
void CDeleteTab4::OnNMDblclkDelete4List(NMHDR* pNMHDR, LRESULT* pResult)
{
	LPNMITEMACTIVATE pNMItemActivate = reinterpret_cast<LPNMITEMACTIVATE>(pNMHDR);
	if (pNMItemActivate->iItem != -1) {
		str = delete4_list.GetItemText(pNMItemActivate->iItem, 4);
		reviewid = _ttoi(str);
...
void CDeleteTab4::OnBnClickedAgreeBtn()
{
	search_agreeid();
...
void CDeleteTab4::OnBnClickedNotAgreeBtn()
{
	search_agreeid();
...
void CDeleteTab4::OnBnClickedGoodBtn()
{
	search_agreeid();
...
//restid를 알아내기
void CDeleteTab4::search_restid() {
	if (!db_connect())
	{
		AfxMessageBox("서버 접속에 실패하여 종료합니다.");
		CDialogEx::OnCancel();
...
bool CDeleteTab4::search_agreeid() {
...
bool CDeleteTab4::insert_agree(int agreetype) {
...
void CDeleteTab4::set_button_color(int agreetype) {
	switch (agreetype) {
	case -1:
		agree.SetFaceColor(RGB(255, 255, 255));
		not_agree.SetFaceColor(RGB(255, 255, 255));
